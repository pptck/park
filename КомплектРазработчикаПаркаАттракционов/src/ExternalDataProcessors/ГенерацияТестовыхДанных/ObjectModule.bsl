...
//  СписокАттракционов - Массив из СправочникСсылка.Атракционы - Список аттракционов
...
Процедура СформироватьПосещенияПосетителя(Посетитель, ДатаПосещения, СписокАттракционов, ГСЧ = Неопределено)
	...
	Для Сч = 1 По КоличествоАттракционов Цикл
		...
		ЗарегистрироватьПосещение(ДатаПосещения, Аттракцион, ОснованиеПосещения);
...
Функция ТекущееОснование(Посетитель, Аттракцион)
	...
	Запрос.Текст = 
		...
		|			Справочник.Атракционы КАК Атракционы
		|		ГДЕ
		|			Атракционы.Ссылка = &Аттракцион)) КАК АктивныеПосещенияОстатки
		|ГДЕ
...
//  Аттракцион - Произвольный, СправочникСсылка.Атракционы - Аттракцион
//  ДатаПосещения - Дата - Дата посещения
//  ГСЧ - ГенераторСлучайныхЧисел, Неопределено - ГСЧ
// 
...
Функция КупитьБилет(Посетитель, Аттракцион, ДатаПосещения, ГСЧ)
	...
	СтрокаТЧ.Цена = РегистрыСведений.ЦеныНоменклатуры.ЦенаНоменклатуры(Номенклатура, ДатаПосещения);
...
Процедура ЗарегистрироватьПосещение(ДатаПосещения, Аттракцион, ОснованиеПосещения);

	ДокОбъект = Документы.ПосещениеАттракциона.СоздатьДокумент();
	ДокОбъект.Заполнить(Неопределено);
	ДокОбъект.Дата = ДатаПосещения;
	ДокОбъект.Атракцион = Аттракцион;
	ДокОбъект.Основание = ОснованиеПосещения;
	
	Если Не ДокОбъект.ПроверитьЗаполнение() Тогда
		ВызватьИсключение "Ошибка создания посещения";
	КонецЕсли;
