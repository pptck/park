...
Процедура СоздатьДекорацииИзбранныхТоваров()
	...
	Для Сч = 1  По КоличествоИзбранныхТоваров Цикл
		...
		ИмяДекорации = "ИмяДекорацияИзбранныхТоваров_" + XMLСтрока(Сч);
...
Процедура ЗаполнитьДекорациюБаллыКлиента()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.БонусныеБаллыКлентов.Остатки(, Клиент = &Клиент) КАК БонусныеБаллыКлентовОстатки";
	...
	Если ОстатокБаллов = 0 Тогда
		Элементы.ДекорацияБаллыКлиента.Заголовок = "У клента нет начисленных баллов";
		Возврат
	КонецЕсли;

	МаксимальнаяДоля = Константы.МаксивальнаяДоляОплатыБаллами.Получить();
	МаксимуБалловКСписанию = Мин(ОстатокБаллов, СуммаИтого * МаксимальнаяДоля / 100);
	
	Шаблон = "Накопленно %1 баллов, из них можно списать %2. ";
	
	Описание = СтрШаблон(Шаблон,
		ОстатокБаллов,
		МаксимуБалловКСписанию);
	
	ШаблонСсылки = "#Заполнить_%1";
	
	СсылкаЗаполненияБаллов = СтрШаблон(ШаблонСсылки,
		XMLСтрока(МаксимуБалловКСписанию));
...
Процедура РассчитатьСуммуДокумента()
	...
	СуммаИтого = ПозицииПродажи.Итог("Сумма");
	ЗаполнитьДекорациюБаллыКлиента();
